...
5. 点 **▶ ① 生成口播视频** → 后台跑第 1 段（约 2~5 分钟，进度在日志框）；**配音会自动适配原视频时长**，实现音画同步
...
`merge_local.py` 做的事：ffprobe 取配音时长与原视频时长 → 文案按句切分均匀铺到字幕时间轴（生成临时 `.srt`）→ ffmpeg 原画面静音 + 烧录字幕 + 混入配音。**最终成品时长以原视频为准**：配音比视频长则按视频时长掐断（不再冻结补长，避免"声音还在念、画面已卡住"），短则补静音。成品与 ChatCut 路径一致：**原视频画面 + 朗读语音 + 同步字幕**。

> **音画同步（2026-07-17 修复）**：本技能里"画面"是原视频（时长固定），配音是 edge-tts 生成的、长短和视频无关。若配音比视频长，合并时 ffmpeg 会用最后一帧冻结补长 → 假同步。修复做法：**第①步生成配音时按原视频时长自动适配语速**（两遍合成：先按用户语速测时长，再按比例把配音时长压到≈原视频时长，语速限定 0.5x~2.0x 防失真），第②步再以原视频时长为基准掐断/补静音。两者等长 → 音画严格对齐。无头命令版可用 `--target-duration <秒>` 触发适配。
